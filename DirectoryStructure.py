...
class DirectoryNode:
    def __init__(self, name, size=0, is_leaf=False):
        ...
        self.type = "folder" if not is_leaf else name.split(".")[1].strip(".")
...
class Directory:
    ...
    def insert_child(self, node, path, child_node):
        ...
        for part in parts[1:]:  # Skip the root, already passed in
            node = next(
                (c for c in node.children if c.name == part and not c.is_leaf), None
            )
            if node is None:
    ...
    def find_type(self, parent, type, total_files, path=""):
        if parent.type == type or (type == "folder" and not parent.is_leaf):
            total_files.append(f"{path}/{parent.name}".lstrip("/"))
    ...
    def _cached_or_sum(self, node):
        ...
        if node.is_leaf:
            ...
            node._cached_total_size = sum(
                self._cached_or_sum(child) for child in node.children
            )
        return node._cached_total_size
...
if __name__ == "__main__":
    ...
    files.insert_child(
        ...
        DirectoryNode(name="File3.txt", size=20, is_leaf=True),
    )
    files.insert_child(root, "Root/Folder2", DirectoryNode(name="SubFolder1"))
    files.insert_child(
        root, "Root/Folder2/SubFolder1", DirectoryNode(name="SubFolder2")
    )
    files.insert_child(
        root,
        "Root/Folder2/SubFolder1/SubFolder2",
        DirectoryNode(name="File4.txt", size=15, is_leaf=True),
    )
    files.insert_child(
        root, "Root/Folder2", DirectoryNode(name="File5.pdf", size=30, is_leaf=True)
    )
    ...
    while True:
        ...
        while len(query) < 2:
            query = input(
                "You must provide a query formatted as such: 'Find file: File3.txt': "
            ).split(":")
        ...
        if query[0].lower() == "find file":
            ...
            print(
                f"Files containing {[None if found_files is None else len(found_files)]}"
            )
        ...
        elif query[0].lower() == "file type":
            print(f"Finding files containing {query[1]}")
            start_time = time.time()
            found_files = files.find_type(root, query[1].strip("."), [])
            print(
                f"Files containing {[None if found_files is None else len(found_files)]}"
            )
            if found_files is not None:
